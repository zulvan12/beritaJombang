...
@section("content")
@if (request('category'))
    <h1 class="mb-3"> {{ $pageTitle }} : {{ ucwords(request('category')) }}</h1>
@else
...
<div class="card mb-3">
    ...
    <div class="card-body text-center">
      ...
      <p class="text-muted">Category : <a class="text-decoration-none" href="/?category={{ $news[0]->category->slug }}">{{ $news[0]->category->name }}</a> | Author : <a class="text-decoration-none" href="/authors/{{ $news[0]->author->username }}">{{ $news[0]->author->name }}</a></p>
